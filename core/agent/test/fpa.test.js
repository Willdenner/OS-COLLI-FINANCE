...
test("aprende padrao de categoria salvo e aplica em import futuro", async () => {
  const dataDir = await createTempDataDir();
  const store = loadFreshStore(dataDir);

  await store.createFpaImport({
    sourceType: "csv",
    originalFilename: "mes-01.csv",
    accountName: "Conta Operacional",
    transactions: [
      {
        accountName: "Conta Operacional",
        transactionDate: "2026-04-01",
        description: "ASSINATURA XPTO SISTEMAS 123",
        amountCents: -9900,
        category: "Não Classificado",
        reportSection: "uncategorized",
        fingerprint: "conta-operacional|2026-04-01|-9900|xpto",
      },
    ],
  });

  const original = (await store.listFpaTransactions({ limit: 10 }))[0];
  await store.updateFpaTransaction(original.id, {
  ...
  const rules = await store.listFpaCategoryRules();
  assert.equal(rules.length, 1);
  assert.equal(rules[0].category, "Serviços e Softwares");

  await store.createFpaImport({
    sourceType: "csv",
    originalFilename: "mes-02.csv",
    accountName: "Conta Operacional",
    transactions: [
      {
        accountName: "Conta Operacional",
        transactionDate: "2026-05-01",
        description: "ASSINATURA XPTO SISTEMAS 456",
        amountCents: -9900,
        category: "Não Classificado",
        reportSection: "uncategorized",
        fingerprint: "conta-operacional|2026-05-01|-9900|xpto",
      },
    ],
  });

  const refreshed = await store.listFpaTransactions({ limit: 10 });
  const imported = refreshed.find((entry) => entry.transactionDate === "2026-05-01");
  const updatedRules = await store.listFpaCategoryRules();

  assert.equal(imported.category, "Serviços e Softwares");
  assert.equal(imported.subcategory, "Assinaturas e parceiros");
  assert.equal(updatedRules[0].matchCount, 1);
});
